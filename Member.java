...
public class Member extends Person{
    ...
    public void addCLassesToSchedule() throws SlotFilledException {
    	...
    	for(WorkoutClass workoutClass : this.registeredClasses) {                         //loop through classes in resgisteredClasses[] array; each class is stored in workoutClass
    		for (int columns = 0; columns < 7 ; columns++) {                              //loop through columns for each WorkoutCourse.weeklySchedule in the ^ array
    			for (int rows = 0; rows < 72; rows++) {                                   //loop through the rows (times)
    				...
    				if(workoutClass.workoutSchedule.WeeklySchedule[columns][rows]) {       //checks if this WorkoutCourse.workoutSchedule slot has a true in it and if so,
    ...
    /*
     *  // to do
     *  add a method to remove classes from schedule
     *  add method that updates the registered classes and calls the addClassToSchedule() or the remove class method. or should this be in the main?
     */
